...
import { agilidadMentalGame } from './games/agilidadmental.js';
...
function ruletaSpin() {
  ...
  // Anti-repetición: juegos recientes tienen 1/4 de probabilidad de salir
  const recent = (tournament && tournament.recentGames) || [];
  const weighted = games.flatMap(g => recent.includes(g.id) ? [g] : [g, g, g, g]);
  const wheel = [...weighted, COMODIN, COMODIN];
...
function onTournamentGameDone(game, ranking) {
  ...
  // Registra juego reciente para anti-repetición de ruleta
  if (!tournament.recentGames) tournament.recentGames = [];
  tournament.recentGames = [game.id, ...tournament.recentGames].slice(0, 3);
