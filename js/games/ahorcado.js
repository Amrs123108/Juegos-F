...
const DIFICULTAD = {
  facil:  { label: 'Fácil',  levels: ['facil'],           rounds: 3, maxErrors: 6, hintPts: 20 },
  media:  { label: 'Media',  levels: ['media'],            rounds: 4, maxErrors: 6, hintPts: 30 },
  dificil:{ label: 'Difícil',levels: ['media','dificil'], rounds: 5, maxErrors: 5, hintPts: 40 },
};

export const ahorcadoGame = {
  ...AHORCADO_CONFIG,
  config: [
    { key: 'dificultad', label: 'Dificultad', type: 'select', default: 'media', options: [
      { value: 'facil',   label: '😊 Fácil – palabras cortas (3 rondas)' },
      { value: 'media',   label: '🎯 Media – frases conocidas (4 rondas)' },
      { value: 'dificil', label: '🔥 Difícil – frases largas, menos vidas (5 rondas)' },
    ]},
  ],
  ...
  start(root, players, cfg, api) {
    ...
    function playWord(item) {
      ...
      function win() {
        ...
        const pts = Math.max(50, 150 - hintsUsed * dif.hintPts);
        teamPointsTotal += pts;
        solved++;
    ...
    function showFinal() {
      ...
      api.logGame(ahorcadoGame.name, `${dif.label} ${solved}/${deck.length} palabras`);
      root.innerHTML = `
