...
export const dibujoGame = {
  ...
  start(root, players, cfg, api) {
    ...
    function playTurn() {
      ...
      let paused = false, pausedRem = time;
      function startTimer(seconds) {
        timer = makeTimer(seconds, {
          dangerAt: 5,
      ...
      // Antes de dar puntos, revela la respuesta y pide confirmar (verificación).
      function confirmHit() {
        if (done || paused) return;
        paused = true;
        pausedRem = timer ? Math.max(1, timer.remaining) : time;
        if (timer) timer.stop();
        if (drawInt) { clearInterval(drawInt); drawInt = null; }
        const ov = document.createElement('div');
        ov.setAttribute('data-overlay', '');
        ov.className = 'fixed inset-0 z-40 bg-slate-900/85 backdrop-blur flex items-center justify-center p-6 animate-pop-in';
        ov.innerHTML = `
          <div class="card p-8 text-center max-w-lg">
            <p class="text-slate-400 text-sm mb-1">El dibujo era…</p>
            <p class="text-4xl md:text-5xl font-display font-extrabold text-sky-300 mb-5">${esc(item.name)}</p>
            <p class="text-slate-300 mb-6">¿La familia dijo exactamente eso?</p>
            <div class="flex gap-3 justify-center flex-wrap">
              <button data-yes class="btn-press px-8 py-4 rounded-2xl bg-emerald-500 text-white text-xl font-extrabold">✅ Sí, acertó</button>
              <button data-no class="btn-press px-8 py-4 rounded-2xl bg-slate-700 text-white text-xl font-extrabold">❌ No, seguir</button>
            </div>
          </div>`;
        root.appendChild(ov);
        ov.querySelector('[data-yes]').onclick = () => { ov.remove(); paused = false; finish(true); };
        ov.querySelector('[data-no]').onclick = () => {
          ov.remove(); paused = false;
          if (!done) { if (drawn < totalSteps) drawInt = setInterval(drawStep, stepMs); startTimer(pausedRem); }
        };
      }

      $('[data-hit]').onclick = confirmHit;
      $('[data-skip]').onclick = () => finish(false);

      function onKey(e) {
        if (paused) return;
        if (e.code === 'Space' || e.key === 'Enter') { e.preventDefault(); confirmHit(); }
